...
:
    eddyViscosity
    <
        RASModel<EddyDiffusivity<phaseCompressibleMyTurbulenceModel>>
    >
    (
        type,
        alpha,
...
{
    // Local references
    const myTwoPhaseSystem& fluid = refCast<const myTwoPhaseSystem>(phase_.fluid());
    ...
    if (!equilibrium_)
    {
        ...
        volScalarField beta
        (
            fluid.lookupSubModel<myDragModel>
            (
                phase_,
