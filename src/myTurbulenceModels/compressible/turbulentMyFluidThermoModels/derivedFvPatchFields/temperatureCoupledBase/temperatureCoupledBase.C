...
#include "turbulentMyFluidThermoModel.H"
...
{
    { KMethodType::mtFluidThermo, "myFluidThermo" },
...
) const
{
    ...
    switch (method_)
    {
        case mtFluidThermo:
        {
            typedef compressible::myTurbulenceModel myTurbulenceModel;

            const word turbName(myTurbulenceModel::propertiesName);

            if
            (
                mesh.foundObject<myTurbulenceModel>(turbName)
            )
            {
                const myTurbulenceModel& turbModel =
                    mesh.lookupObject<myTurbulenceModel>(turbName);

                return turbModel.kappaEff(patchi);
            }
            else if (mesh.foundObject<myFluidThermo>(basicThermo::dictName))
            {
                const myFluidThermo& thermo =
                    mesh.lookupObject<myFluidThermo>(basicThermo::dictName);
