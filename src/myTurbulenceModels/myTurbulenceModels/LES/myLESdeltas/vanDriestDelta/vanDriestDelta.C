...
namespace LESModels
{
    ...
    addToRunTimeSelectionTable(myLESdelta, vanDriestDelta, dictionary);
}
}
...
{
    const fvMesh& mesh = myTurbulenceModel_.mesh();

    const volVectorField& U = myTurbulenceModel_.U();
    const tmp<volScalarField> tnu = myTurbulenceModel_.nu();
    const volScalarField& nu = tnu();
    tmp<volScalarField> nuSgs = myTurbulenceModel_.nut();
...
{
    if (myTurbulenceModel_.mesh().time().timeIndex() % calcInterval_ == 0)
    {
